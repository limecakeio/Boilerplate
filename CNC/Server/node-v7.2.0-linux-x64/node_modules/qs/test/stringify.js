...
test('stringify()', function (t) {
	...
	t.test('stringifies a date', function (st) {
		let now = new Date();
		let str = 'a=' + encodeURIComponent(now.toISOString());
	...
	t.test('doesn\'t blow up when Buffer global is missing', function (st) {
		let tempBuffer = global.Buffer;
		delete global.Buffer;
		let result = qs.stringify({ a: 'b', c: 'd' });
	...
	t.test('supports custom representations when filter=function', function (st) {
		let calls = 0;
		let obj = { a: 'b', c: 'd', e: { f: new Date(1257894000000) } };
		let filterFunc = function (prefix, value) {
			calls++;
			if (calls === 1) {
	...
	t.test('can stringify with custom encoding', function (st) {
		st.equal(qs.stringify({ 県: '大阪府', '': '' }, {
			encoder: function (str) {
				...
				let buf = iconv.encode(str, 'shiftjis');
				let result = [];
