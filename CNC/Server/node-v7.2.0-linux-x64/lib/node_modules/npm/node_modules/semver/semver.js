...
/* nomin */ if (typeof process === 'object' &&
	...
	/* nomin */ debug = function() {
		/* nomin */ let args = Array.prototype.slice.call(arguments, 0);
...
function clean(version, loose) {
	let s = parse(version.trim().replace(/^[=v]+/, ''), loose);
...
function SemVer(version, loose) {
	...
	this.loose = loose;
	let m = version.trim().match(loose ? re[LOOSE] : re[FULL]);
...
Comparator.prototype.parse = function(comp) {
	let r = this.loose ? re[COMPARATORLOOSE] : re[COMPARATOR];
	let m = comp.match(r);
...
Range.prototype.parseRange = function(range) {
	let loose = this.loose;
	range = range.trim();
	debug('range', range, loose);
  // `1.2.3 - 1.2.4` => `>=1.2.3 <=1.2.4`
	let hr = loose ? re[HYPHENRANGELOOSE] : re[HYPHENRANGE];
  ...
  // ready to be split into comparators.
	...
	let compRe = loose ? re[COMPARATORLOOSE] : re[COMPARATOR];
...
function replaceTilde(comp, loose) {
	let r = loose ? re[TILDELOOSE] : re[TILDE];
...
function replaceCaret(comp, loose) {
	...
	let r = loose ? re[CARETLOOSE] : re[CARET];
	return comp.replace(r, function(_, M, m, p, pr) {
...
function replaceXRange(comp, loose) {
	comp = comp.trim();
	let r = loose ? re[XRANGELOOSE] : re[XRANGE];
...
function prerelease(version, loose) {
	let parsed = parse(version, loose);
