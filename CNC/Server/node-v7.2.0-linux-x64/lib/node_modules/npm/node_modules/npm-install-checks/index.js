...
function checkEngine (target, npmVer, nodeVer, force, strict, cb) {
	let nodev = force ? null : nodeVer;
	let eng = target.engines;
	if (!eng) return cb();
	if (nodev && eng.node && !semver.satisfies(nodev, eng.node) ||
      eng.npm && !semver.satisfies(npmVer, eng.npm)) {
		let er = new Error(util.format('Unsupported engine for %s: wanted: %j (current: %j)',
...
function checkPlatform (target, force, cb) {
	...
	if (!osOk || !cpuOk) {
		let er = new Error(util.format('Unsupported platform for %s: wanted %j (current: %j)',
...
function checkCycle (target, ancestors, cb) {
  ...
  // is used *twice* here: to skip the first level of repetition.
	...
	let p = Object.getPrototypeOf(Object.getPrototypeOf(ancestors));
	...
	let er = new Error(target._id + ': Unresolvable cycle detected');
	let tree = [ target._id, JSON.parse(JSON.stringify(ancestors)) ];
	let t = Object.getPrototypeOf(ancestors);
...
function checkGit_ (folder, cb) {
	fs.stat(path.resolve(folder, '.git'), function (er, s) {
		if (!er && s.isDirectory()) {
			let e = new Error(folder + ': Appears to be a git repo or submodule.');
			e.path = folder;
			e.code = 'EISGIT';
			return cb(e);
		}
