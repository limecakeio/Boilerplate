...
let readdir = require('graceful-fs').readdir;
let rmdir = require('graceful-fs').rmdir;
let unlink = require('graceful-fs').unlink;
...
function vacuum (leaf, options, cb) {
	...
	let log = options.log ? options.log : function () {};

	leaf = leaf && resolve(leaf);
	let base = options.base && resolve(options.base);
    ...
    // either we've reached the base or we've reached the root
		...
		readdir(branch, function (error, files) {
			...
			lstat(branch, function (error, stat) {
				...
				let remove = stat.isDirectory() ? rmdir : unlink;
