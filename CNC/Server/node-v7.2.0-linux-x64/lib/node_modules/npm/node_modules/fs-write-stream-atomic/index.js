...
let setImmediate = global.setImmediate || setTimeout;
...
WriteStreamAtomic.prototype._write = function (buffer, encoding, cb) {
	let flushed = this.__atomicStream.write(buffer, encoding);
...
function handleClose (writeStream) {
	return function () {
		...
		if (writeStream.__atomicChown) {
			let uid = writeStream.__atomicChown.uid;
			let gid = writeStream.__atomicChown.gid;
