...
function finish (self) {
	...
	for (var i = 0, l = self.matches.length; i < l; i++) {
        ...
        // do like the shell, and spit out the literal glob
				let literal = self.minimatch.globSet[i];
				if (nou)
					all.push(literal);
				else
          all[literal] = true;
	...
	if (self.mark) {
		...
		if (self.nodir) {
			all = all.filter(function (e) {
				let notDir = !(/\/$/.test(e));
				let c = self.cache[e] || self.cache[makeAbs(self, e)];
				if (notDir && c)
...
function mark (self, p) {
	let abs = makeAbs(self, p);
	let c = self.cache[abs];
	let m = p;
	if (c) {
