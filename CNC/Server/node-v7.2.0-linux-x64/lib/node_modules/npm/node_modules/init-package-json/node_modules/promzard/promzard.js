...
promzard.fromBuffer = function (buf, ctx, cb) {
	...
	files[filename] = buf;
	let ret = promzard(filename, ctx, cb);
...
PromZard.prototype.loaded = function () {
	...
	let mod = this.ctx.module = this.makeModule();
	...
	let fn = vm.runInThisContext(this.script, this.file);
	let args = Object.keys(this.ctx).map(function (k) {
		return this.ctx[k];
	}.bind(this));
	try {
...
PromZard.prototype.makeModule = function () {
	let mod = new Module(this.file, module);
	mod.loaded = true;
...
PromZard.prototype.wrap = function (body) {
	let s = '(function( %s ) { %s\n })';
	let args = Object.keys(this.ctx).join(', ');
	...
	function L () {
		...
		while (i < len) {
			...
			if (v && typeof v === 'object') {
				...
				let n = +v.substr(this.unique.length + 1);
				let prompt = this.prompts[n];
				if (isNaN(n) || !prompt)
					continue;
