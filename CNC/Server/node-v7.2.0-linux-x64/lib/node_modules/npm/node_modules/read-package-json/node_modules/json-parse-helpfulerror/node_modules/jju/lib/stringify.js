...
let escapable = /[\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/;

function _stringify(object, options, recursiveLvl, currentKey) {
	let json5 = (options.mode === 'json5' || !options.mode);
  ...
  // if !add, it's an ending indentation, so we just indent
	function indent(str, add) {
		let prefix = options._prefix ? options._prefix : '';
		if (!options.indent) return prefix + str;
		let result = '';
		let count = recursiveLvl + (add || 0);
		for (let i = 0; i < count; i++) result += options.indent;
	...
	function _stringify_str(key) {
		...
		let quoteChr = quote.charCodeAt(0);
	...
	function _stringify_object() {
		...
		if (Array.isArray(object)) {
			...
			for (var i = 0; i < object.length; i++) {
				let s = _stringify(object[i], options, recursiveLvl + 1, String(i));
			...
			braces = '{}';
			let fn = function(key) {
				let t = _stringify(object[key], options, recursiveLvl + 1, key);
				if (t !== undefined) {
