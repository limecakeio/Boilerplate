...
let tokenize = require('./parse').tokenize;
let stringify = require('./stringify').stringify;
let analyze = require('./analyze').analyze;
...
function value_to_tokenlist(value, stack, options, is_key, indent) {
	...
	let stringified = stringify(value, options);
	...
	let result = tokenize(stringified, {
		_addstack: stack
	});
	result.data = null;
	return result;
...
function Document(text, options) {
	let self = Object.create(Document.prototype);

	if (options == null) options = {};
  //options._structure = true
	let tokens = self._tokens = tokenize(text, options);
	self._data = tokens.data;
	tokens.data = null;
	self._options = options;

	let stats = analyze(text, options);
