...
let safeJSON = require('json-parse-helpfulerror');
...
function indexjs (file, er, log, strict, cb) {
	...
	let index = path.resolve(path.dirname(file), 'index.js');
...
function scriptpath (file, data, cb) {
	...
	let k = Object.keys(data.scripts);
...
function authors (file, data, cb) {
	...
	let af = path.resolve(path.dirname(file), 'AUTHORS');
...
function readme (file, data, cb) {
	...
	let dir = path.dirname(file);
	let globOpts = { cwd: dir, nocase: true, mark: true };
    ...
    // don't accept directories.
		...
		let fn = preferMarkdownReadme(files);
		let rm = path.resolve(dir, fn);
...
function mans (file, data, cb) {
	let m = data.directories && data.directories.man;
...
function bins (file, data, cb) {
	...
	let m = data.directories && data.directories.bin;
...
function githead (file, data, cb) {
	...
	let dir = path.dirname(file);
	let head = path.resolve(dir, '.git/HEAD');
...
function githead_ (file, data, dir, head, cb) {
	...
	let headFile = head.replace(/^ref: /, '').trim();
...
function checkBinReferences_ (file, data, warn, cb) {
	...
	keys.forEach(function (key) {
		...
		let relName = data.bin[key];
		let binPath = path.resolve(dirName, relName);
		fs.exists(binPath, handleExists.bind(null, relName));
	});
}

function final (file, data, log, strict, cb) {
	let pId = makePackageId(data);
...
function makePackageId (data) {
	let name = cleanString(data.name);
	let ver = cleanString(data.version);
	return name + '@' + ver;
}
...
function parseError (ex, file) {
	let e = new Error('Failed to parse json\n' + ex.message);
	e.code = 'EJSONPARSE';
	e.file = file;
	return e;
}
