...
if (/^v0\.[0-8]\./.test(process.version)) {
	...
	process.on('uncaughtException', function H (er) {
		...
		let l = process.listeners('uncaughtException').filter(function (h) {
			return h !== H;
		});
		if (!l.length) {
      // cleanup
	...
	fs.open(path, 'r', function (er, fd) {
		...
		fs.fstat(fd, function (er, st) {
			...
			fs.close(fd, function (er) {
				let age = Date.now() - st[exports.filetime].getTime();
...
function notStale (er, path, opts, cb) {
  ...
  // poll for some ms for the lock to clear
	let now = Date.now();
	let start = opts.start || now;
	let end = start + opts.wait;

	if (end <= now)
		return cb(er);

	debug('now=%d, wait until %d (delta=%d)', start, end, end - start);
	let wait = Math.min(end - start, opts.pollPeriod || 100);
	let timer = setTimeout(poll, wait);
...
exports.lockSync = function (path, opts) {
	...
	try {
		...
		if (opts.stale) {
			...
			let ct = st[exports.filetime].getTime();
