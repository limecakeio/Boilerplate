...
let MurmurHash3 = require('imurmurhash');
let extend = Object.assign || require('util')._extend;

function murmurhex () {
	let hash = new MurmurHash3();
	for (let ii = 0; ii < arguments.length; ++ii) hash.hash('' + arguments[ii]);
	return hash.result();
}
let invocations = 0;
let getTmpname = function (filename) {
...
module.exports = function writeFile (filename, data, options, callback) {
	...
	if (!options) options = {};
	let tmpfile = getTmpname(filename);
