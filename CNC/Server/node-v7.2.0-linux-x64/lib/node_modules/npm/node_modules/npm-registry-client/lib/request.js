...
function regRequest (uri, params, cb_) {
  ...
  // don't let the cb be called more than once.
	...
	let adduserChange = /\/?-\/user\/org\.couchdb\.user:([^/]+)\/-rev/;
	let isUserChange = uri.match(adduserChange);
	let adduserNew = /\/?-\/user\/org\.couchdb\.user:([^/?]+)$/;
	let isNewUser = uri.match(adduserNew);
	let alwaysAuth = params.auth && params.auth.alwaysAuth;
	let isDelete = params.method === 'DELETE';
	let isWrite = params.body || isDelete;
  ...
  // new users can *not* use auth, because they don't *have* auth yet
	...
	this.attempt(function (operation) {
      ...
      // Only retry on 408, 5xx or no `response`.
			...
			let timeout = statusCode === 408;
			let serverError = statusCode >= 500;
			let statusRetry = !statusCode || timeout || serverError;
...
function makeRequest (uri, params, cb_) {
	let socket;
	let cb = once(function (er, parsed, raw, response) {
		if (socket) {
  ...
  // metadata should be compressed
	...
	let er = this.authify(params.authed, parsed, headers, params.auth);
	if (er) return cb_(er);

	let opts = this.initialize(
    parsed,
    params.method,
    'application/json',
    headers
  ...
  // figure out wth body is
	...
	let done = requestDone.call(this, params.method, uri, cb);
	let req = request(opts, params.streaming ? undefined : decodeResponseBody(done));
...
function requestDone (method, where, cb) {
    ...
    // for the search endpoint, the 'error' property can be an object
        ...
        response.statusCode >= 400) {
			let w = url.parse(where).pathname.substr(1);
			let name;
			if (!w.match(/^-/)) {
				w = w.split('/');
