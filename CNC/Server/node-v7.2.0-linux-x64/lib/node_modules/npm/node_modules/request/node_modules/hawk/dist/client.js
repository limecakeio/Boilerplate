...
exports.header = function (uri, method, options) {
	...
	let timestamp = options.timestamp || Utils.nowSecs(options.localtimeOffsetMsec);
...
exports.authenticate = function (res, credentials, artifacts, options) {
	...
	if (res.headers['www-authenticate']) {
		...
		let wwwAttributes = Utils.parseAuthorizationHeader(res.headers['www-authenticate'], [ 'ts', 'tsm', 'error' ]);
		...
		if (wwwAttributes.ts) {
			let tsm = Crypto.calculateTsMac(wwwAttributes.ts, credentials);
	...
	let attributes = Utils.parseAuthorizationHeader(res.headers['server-authorization'], [ 'mac', 'ext', 'hash' ]);
	...
	let calculatedHash = Crypto.calculatePayloadHash(options.payload, credentials.algorithm, res.headers['content-type']);
...
exports.getBewit = function (uri, options) {
	...
	let exp = Math.floor(now / 1000) + options.ttlSec;
