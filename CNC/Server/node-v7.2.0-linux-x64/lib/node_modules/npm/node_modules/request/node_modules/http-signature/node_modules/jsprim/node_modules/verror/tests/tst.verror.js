...
let mod_verror = require('../lib/verror');

let VError = mod_verror.VError;
let WError = mod_verror.WError;

let err, suberr, stack, substack;
...
function cleanStack(stacktxt) {
	let re = new RegExp(__filename + ':\\d+:\\d+', 'gm');
...
let nodestack = new Error().stack.split('\n').slice(2).join('\n');
