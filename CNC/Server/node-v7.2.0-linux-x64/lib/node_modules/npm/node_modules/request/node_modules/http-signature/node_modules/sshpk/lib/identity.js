...
let DNS_NAME_RE = /^([*]|[a-z0-9][a-z0-9\-]{0,62})(?:\.([*]|[a-z0-9][a-z0-9\-]{0,62}))*$/i;
...
Identity.parseDN = function (dn) {
	assert.string(dn, 'dn');
	let parts = dn.split(',');
	let cmps = parts.map(function (c) {
		c = c.trim();
		let eqPos = c.indexOf('=');
		let name = c.slice(0, eqPos).toLowerCase();
		let value = c.slice(eqPos + 1);
