...
function textwrap(s, width) {
	...
	words.forEach(function (w) {
		let newLength = line.length + w.length;
		if (line.length > 0)
			newLength += 1;
		if (newLength > width) {
			lines.push(line);
...
Parser.prototype.parse = function parse(inputs) {
	...
	let argv = inputs.argv || process.argv;
	let slice = inputs.slice !== undefined ? inputs.slice : 2;
	let args = argv.slice(slice);
	let env = inputs.env || process.env;
	let opts = {};
	let _order = [];

	function addOpt(option, optstr, key, val, from) {
		let type = optionTypes[option.type];
		let parsedVal = type.parseArg(option, optstr, val);
		if (type.array) {
			if (!opts[key]) {
				opts[key] = [];
			}
		...
		let item = { key: key, value: parsedVal, from: from };
		_order.push(item);
	}
	...
	Object.keys(this.optionFromEnv).forEach(function (envname) {
		...
		if (val === undefined)
			return;
		let option = self.optionFromEnv[envname];
		if (opts[option.key] !== undefined)
			return;
		let takesArg = self.optionTakesArg(option);
		if (takesArg) {
...
Parser.prototype.help = function help(config) {
	...
	let indent = makeIndent(config.indent, 4, 'config.indent');
	let headingIndent = makeIndent(config.headingIndent,
        Math.round(indent.length / 2), 'config.headingIndent');

	assert.optionalString(config.nameSort, 'config.nameSort');
	let nameSort = config.nameSort || 'length';
	...
	let maxCol = config.maxCol || 80;
	let minHelpCol = config.minHelpCol || 20;
	let maxHelpCol = config.maxHelpCol || 40;
	...
	this.options.forEach(function (o) {
		...
		let type = optionTypes[o.type];
		let arg = o.helpArg || type.helpArg || 'ARG';
		let line = '';
	...
	this.options.forEach(function (o) {
		...
		let line = lines[i] = indent + lines[i];
		if (!o.help && !(config.includeEnv && o.env) && !helpDefault) {
			return;
		}
		let n = helpCol - line.length;
		if (n >= 0) {
			line += space(n);
		} else {
			line += '\n' + space(helpCol);
		...
		if (o.env && o.env.length && config.includeEnv) {
			...
			let type = optionTypes[o.type];
			let arg = o.helpArg || type.helpArg || 'ARG';
			let envs = (Array.isArray(o.env) ? o.env : [ o.env ]).map(
                function (e) {
	if (type.takesArg) {
		return e + '=' + arg;
	} else {
...
function bashCompletionSpecFromOptions(args) {
	...
	let includeHidden = (args.includeHidden === undefined
	...
	(args.options || []).forEach(function (o) {
		...
		let optNames = o.names || [ o.name ];
		let optType = getOptionType(o.type);
		if (optType.takesArg) {
			let completionType = o.completionType ||
...
function bashCompletionFromOptions(args) {
    ...
    // Render template.
	let template = fs.readFileSync(BASH_COMPLETION_TEMPLATE_PATH, 'utf8');
...
function synopsisFromOpt(o) {
	...
	let names = o.names || [ o.name ];
    // `type` here could be undefined if, for example, the command has a
    // dashdash option spec with a bogus 'type'.
	let type = getOptionType(o.type);
	let helpArg = o.helpArg || (type && type.helpArg) || 'ARG';
	let parts = [];
	names.forEach(function (name) {
		let part = (name.length === 1 ? '-' : '--') + name;
