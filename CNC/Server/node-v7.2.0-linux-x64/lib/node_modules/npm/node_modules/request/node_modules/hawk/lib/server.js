...
exports.authenticate = function (req, credentialsFunc, options, callback) {
	...
	let now = Utils.now(options.localtimeOffsetMsec);                           // Measure now before any other processing

    // Convert node Http request object to a request configuration object

	let request = Utils.parseRequest(req, options);
	...
	let attributes = Utils.parseAuthorizationHeader(request.authorization);
	...
	credentialsFunc(attributes.id, function (err, credentials) {
		...
		let mac = Crypto.calculateMac('header', credentials, artifacts);
		...
		if (options.payload ||
			...
			let hash = Crypto.calculatePayloadHash(options.payload, credentials.algorithm, request.contentType);
		...
		options.nonceFunc(credentials.key, attributes.nonce, attributes.ts, function (err) {
			...
			if (Math.abs((attributes.ts * 1000) - now) > (options.timestampSkewSec * 1000)) {
				let tsm = Crypto.timestampMessage(credentials, options.localtimeOffsetMsec);
...
exports.authenticatePayload = function (payload, credentials, artifacts, contentType) {
	...
	let calculatedHash = Crypto.calculatePayloadHash(payload, credentials.algorithm, contentType);
...
exports.header = function (credentials, artifacts, options) {
	...
	let mac = Crypto.calculateMac('response', credentials, artifacts);
...
exports.authenticateBewit = function (req, credentialsFunc, options, callback) {
	...
	let now = Utils.now(options.localtimeOffsetMsec);
	...
	let resource = request.url.match(internals.bewitRegex);
	if (!resource) {
	...
	let bewitString = Hoek.base64urlDecode(resource[3]);
	...
	let bewitParts = bewitString.split('\\');
	...
	credentialsFunc(bewit.id, function (err, credentials) {
		...
		let mac = Crypto.calculateMac('bewit', credentials, {
			ts: bewit.exp,
			nonce: '',
			method: 'GET',
			resource: url,
...
exports.authenticateMessage = function (host, port, message, authorization, credentialsFunc, options, callback) {
	...
	credentialsFunc(authorization.id, function (err, credentials) {
		...
		let mac = Crypto.calculateMac('message', credentials, artifacts);
		...
		let hash = Crypto.calculatePayloadHash(message, credentials.algorithm);
