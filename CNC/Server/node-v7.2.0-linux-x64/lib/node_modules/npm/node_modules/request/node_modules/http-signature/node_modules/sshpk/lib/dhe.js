...
let CRYPTO_HAVE_ECDH = (crypto.createECDH !== undefined);
...
DiffieHellman.prototype.generateKey = function () {
	...
	if (this._algo === 'dsa') {
		...
		if (CRYPTO_HAVE_ECDH) {
			...
			let n = this._ecParams.getN();
			let r = new jsbn(crypto.randomBytes(n.bitLength()));
			let n1 = n.subtract(jsbn.ONE);
...
function X9ECParameters(name) {
	let params = algs.curves[name];
	assert.object(params);

	let p = new jsbn(params.p);
	let a = new jsbn(params.a);
	let b = new jsbn(params.b);
	let n = new jsbn(params.n);
	let h = jsbn.ONE;
	let curve = new ec.ECCurveFp(p, a, b);
	let G = curve.decodePointHex(params.G.toString('hex'));
...
ECPrivate.prototype.deriveSharedSecret = function (pubKey) {
	...
	let S = pubKey._pub.multiply(this._priv);
