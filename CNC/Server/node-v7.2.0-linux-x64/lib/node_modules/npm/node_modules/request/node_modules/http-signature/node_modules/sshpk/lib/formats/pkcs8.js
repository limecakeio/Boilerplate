...
function readPkcs8(alg, type, der) {
	...
	der.readSequence();
	let next = der.offset + der.length;
...
function readPkcs8DSAPrivate(der) {
	...
	let y = utils.calculateDSAPublic(g, p, x);
...
function readECDSACurve(der) {
	...
	if (der.peek() === asn1.Ber.OID) {
		...
		der.readSequence();
		let version = der.readString(asn1.Ber.Integer, true);
		...
		// FieldID sequence
		der.readSequence();
		let fieldTypeOid = der.readOID();
		assert.strictEqual(fieldTypeOid, '1.2.840.10045.1.1',
		    'ECDSA key is not from a prime-field');
		let p = curve.p = utils.mpNormalize(
		    der.readString(asn1.Ber.Integer, true));
...
function readPkcs8ECDSAPrivate(der) {
	let curveName = readECDSACurve(der);
...
function writeECDSACurve(key, der) {
	let curve = algs.curves[key.curve];
	if (curve.pkcs8oid) {
