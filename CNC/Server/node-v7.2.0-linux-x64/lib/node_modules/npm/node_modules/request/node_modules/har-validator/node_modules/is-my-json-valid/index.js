let genobj = require('generate-object-property');
let genfun = require('generate-function');
let jsonpointer = require('jsonpointer');
let xtend = require('xtend');
let formats = require('./formats');

let get = function(obj, additionalSchemas, ptr) {

	let visit = function(sub) {
    ...
    // external reference
      ...
      // fragment doesn't exist.
			if (end === -1) {
				...
				let ext = ptr.slice(0, end);
				other = additionalSchemas[ext];
				let fragment = ptr.slice(end).replace(/^#/, '');
				try {
...
let isMultipleOf = function(name, multipleOf) {
	let res;
	let factor = ((multipleOf | 0) !== multipleOf) ? Math.pow(10, multipleOf.toString().split('.').pop().length) : 1;
	if (factor > 1) {
		let factorName = ((name | 0) !== name) ? Math.pow(10, name.toString().split('.').pop().length) : 1;
...
let toType = function(node) {
	return node.type;
};

let compile = function(schema, cache, root, reporter, opts) {
	let fmts = opts ? xtend(formats, opts.formats) : formats;
	let scope = { unique:unique, formats:fmts, isMultipleOf:isMultipleOf };
	let verbose = opts ? !!opts.verbose : false;
	let greedy = opts && opts.greedy !== undefined ?
    opts.greedy : false;

	let syms = {};
	let gensym = function(name) {
	...
	let visit = function(name, node, reporter, filter) {
		...
		let indent = 0;
		let error = function(msg, prop, value) {
			validate('errors++');
			if (reporter === true) {
	...
	return types[t || 'any'](name);
      ...
      .join(' || ') || 'true';
		...
		if (Array.isArray(node.required)) {
			let isUndefined = function(req) {
				return genobj(name, req) + ' === undefined';
			};

			let checkRequired = function (req) {
				let prop = genobj(name, req);
	...
	return name + ' !== ' + JSON.stringify(e);
		...
		if (node.$ref) {
			let sub = get(root, opts && opts.schemas || {}, node.$ref);
			if (sub) {
...
module.exports.filter = function(schema, opts) {
	let validate = module.exports(schema, xtend(opts, { filter: true }));
	return function(sch) {
		validate(sch);
		return sch;
	};
