...
function padCenter(str, max, chr) {
	...
	let lengthRight = length - lengthLeft;
...
function splitLongWords(str, max, truncationChar) {
	...
	let truncationWidth = wcwidth(truncationChar);
	...
	while (remainder || words.length) {
      ...
      // slice is based on length no wcwidth
			let i = 0;
			let wwidth = 0;
			let limit = max - truncationWidth;
			while (i < word.length) {
				let w = wcwidth(word.charAt(i));
				if (w + wwidth > limit) {
					break;
				}
				wwidth += w;
