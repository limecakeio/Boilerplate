...
let HostedGit = require('hosted-git-info');

module.exports = npa;

let isWindows = process.platform === 'win32' || global.FAKE_WINDOWS;
let slashRe = isWindows ? /\\|[/]/ : /[/]/;

let parseName = /^(?:@([^/]+?)[/])?([^/]+?)$/;
let nameAt = /^(@([^/]+?)[/])?([^/]+?)@/;
let debug = util.debuglog
  ? util.debuglog('npa')
...
function npa (arg) {
  ...
  // See if it's something like foo@...
	let nameparse = arg.match(nameAt);
  ...
  // if it's got / chars in it, then assume that it's local.
	...
	if (res.name) {
		if (arg === '') arg = 'latest';
		let version = semver.valid(arg, true);
		let range = semver.validRange(arg, true);
    // foo@...
		if (version) {
			res.spec = version;
			res.type = 'version';
...
function parseUrl (res, arg, urlparse) {
	let gitHost = HostedGit.fromUrl(arg);
	if (gitHost) {
		res.type = 'hosted';
