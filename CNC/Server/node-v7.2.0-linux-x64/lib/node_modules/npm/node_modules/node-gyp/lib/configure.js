...
function configure (gyp, argv, callback) {
	...
	function createConfigFile (err) {
		...
		let configFilename = 'config.gypi';
		let configPath = path.resolve(buildDir, configFilename);
    ...
    // now write out the config.gypi file to the build/ dir
		let prefix = '# Do not edit. File was generated by node-gyp\'s "configure" step',
	...
	function findConfigs (err) {
		if (err) return callback(err);
		let name = configNames.shift();
		if (!name) return runGyp();
		let fullPath = path.resolve(name);
    ...
    //  - the root directory
		let node_exp_file = undefined;
		if (process.platform === 'aix') {
			let node_root_dir = findNodeDirectory();
			let candidates = [ 'include/node/node.exp',
			...
			if (node_exp_file !== undefined) {
				...
				let msg = msgFormat('Could not find node.exp file in %s', node_root_dir);
    ...
    // this logic ported from the old `gyp_addon` python file
		let gyp_script = path.resolve(__dirname, '..', 'gyp', 'gyp_main.py');
		let addon_gypi = path.resolve(__dirname, '..', 'addon.gypi');
		let common_gypi = path.resolve(nodeDir, 'include/node/common.gypi');
        ...
        // Windows expects an absolute path
			...
			let nodeGypDir = path.resolve(__dirname, '..');
      ...
      // make sure python uses files that came with this particular node package
			let pypath = new PathArray(process.env, 'PYTHONPATH');
			pypath.unshift(path.join(__dirname, '..', 'gyp', 'pylib'));

			let cp = gyp.spawn(python, argv);
			cp.on('exit', onCpExit);
		});
	}
...
function findAccessibleSync (logprefix, dir, candidates) {
	for (let next = 0; next < candidates.length; next++) {
		let candidate = path.resolve(dir, candidates[next]);
		try {
...
function findPython (python, callback) {
  ...
  // where the actual "python.exe" executable is located.
	function checkPythonLauncher () {
		...
		let launcherArgs = [ '-2', '-c', 'import sys; print sys.executable' ];
  ...
  // We're gonna check if "%SystemDrive%\python27\python.exe" exists.
	function guessPython () {
		...
		let pythonPath = path.resolve(rootDir, 'Python27', 'python.exe');
	...
	function checkPythonVersion () {
		...
		execFile(python, [ '-c', 'import platform; print(platform.python_version());' ], { env: env }, function (err, stdout) {
			...
			let range = semver.Range('>=2.5.0 <3.0.0');
			let valid = false;
			try {
