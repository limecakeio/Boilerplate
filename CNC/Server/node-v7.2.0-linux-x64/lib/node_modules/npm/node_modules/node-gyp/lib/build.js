...
function build (gyp, argv, callback) {
   ...
   */
	...
	function findMsbuild () {
		...
		let notfoundErr = 'Can\'t find "msbuild.exe". Do you have Microsoft Visual Studio C++ 2008+ installed?';
   ...
   */
	...
	function copyNodeLib () {
		...
		let buildDir = path.resolve(nodeDir, buildType),
		...
		mkdirp(buildDir, function (err, isNew) {
			...
			let rs = fs.createReadStream(archNodeLibPath),
   ...
   */
    ...
    // Enable Verbose build
		let verbose = log.levels[log.level] <= log.levels.verbose;
		if (!win && verbose) {
			argv.push('V=1');
		}
		if (win && !verbose) {
    ...
    // Specify the build type, Release by default
      ...
      // did the user specify their own .sln file?
			let hasSln = argv.some(function (arg) {
		...
		let proc = gyp.spawn(command, argv);
		proc.on('exit', onExit);
	}
