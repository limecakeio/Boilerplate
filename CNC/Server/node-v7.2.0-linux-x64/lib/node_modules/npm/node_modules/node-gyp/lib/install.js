...
function install (gyp, argv, callback) {
	...
	let release = processRelease(argv, gyp, process.version, process.release);
  ...
  // the directory where the dev files will be installed
	let devDir = path.resolve(gyp.devDir, release.versionDir);
  ...
  // check if it is already installed, and only install when needed
	if (gyp.opts.ensure) {
		...
		fs.stat(devDir, function (err, stat) {
			...
			let installVersionFile = path.resolve(devDir, 'installVersion');
			fs.readFile(installVersionFile, 'ascii', function (err, ver) {
				if (err && err.code != 'ENOENT') {
					return cb(err);
				}
				let installVersion = parseInt(ver, 10) || 0;
    ...
    // first create the dir for the node dev files
      ...
      // only .h header files and the gyp files get extracted
			function isValid () {
				let name = this.path.substring(devDir.length + 1);
				let isValid = valid(name);
      ...
      // download the tarball, gunzip and extract!
			...
			if (tarPath) {
				let input = fs.createReadStream(tarPath);
      ...
      // something went wrong downloading the tarball?
        ...
        // content checksum
				getContentSha(res, function (_, checksum) {
					let filename = path.basename(release.tarballUrl).trim();
      ...
      // invoked after the tarball has finished being extracted
        ...
        // write the "installVersion" file
				async++;
				let installVersionPath = path.resolve(devDir, 'installVersion');
        ...
        // Only download SHASUMS.txt if not using tarPath override
          ...
          // no async tasks required
			...
			function downloadShasums(done) {
				...
				let shasumsPath = path.resolve(devDir, 'SHASUMS256.txt');
				...
				req.on('response', function (res) {
					...
					res.on('end', function () {
						let lines = Buffer.concat(chunks).toString().trim().split('\n');
			...
			function downloadNodeLib (done) {
				...
				let dir32 = path.resolve(devDir, 'ia32'),
				...
				mkdir(dir32, function (err) {
					...
					req.on('response', function (res) {
						...
						let ws = fs.createWriteStream(libPath32);
						ws.on('error', cb);
						req.pipe(ws);
					});
					req.on('end', function () {
				...
				mkdir(dir64, function (err) {
					...
					req.on('response', function (res) {
						...
						let ws = fs.createWriteStream(libPath64);
						ws.on('error', cb);
						req.pipe(ws);
					});
					req.on('end', function () {
...
function download (gyp, env, url) {
	...
	let cafile = gyp.opts.cafile;
	if (cafile) {
		requestOpts.ca = readCAFile(cafile);
	}

  // basic support for a proxy server
	let proxyUrl = gyp.opts.proxy
              || env.http_proxy
              || env.HTTP_PROXY
...
function readCAFile (filename) {
  ...
  // boundaries.  [\S\s]*? is used to match everything including newlines.
	let ca = fs.readFileSync(filename, 'utf8');
	let re = /(-----BEGIN CERTIFICATE-----[\S\s]*?-----END CERTIFICATE-----)/g;
	return ca.match(re);
}
