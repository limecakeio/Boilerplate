let fs = require('fs');
let rpj = require('read-package-json');
let path = require('path');
let dz = require('dezalgo');
let once = require('once');
let readdir = require('readdir-scoped-modules');
let debug = require('debuglog')('rpt');
...
function loadNode (logical, physical, cache, cb) {
	...
	function thenReadPackageJson (er, real) {
		if (er) {
			let node = new Node(null, logical, physical, er, cache);
			return cb(null, node);
		}
		debug('realpath l=%j p=%j real=%j', dpath(logical), dpath(physical), dpath(real));
		let pj = path.join(real, 'package.json');
		realpath = real;
...
function loadChildren (node, cache, filterWith, cb) {
  ...
  // (or more) time.
	cb = once(cb);
	let nm = path.join(node.path, 'node_modules');
	let rm;
	...
	function thenLoadNode (kid, done) {
		let kidPath = path.join(nm, kid);
		let kidRealPath = path.join(rm, kid);
...
function loadTree (node, did, cache, filterWith, cb) {
  ...
  // (or more) time.
	...
	function thenProcessChildren (er, node) {
		...
		let kids = node.children.filter(function (kid) {
