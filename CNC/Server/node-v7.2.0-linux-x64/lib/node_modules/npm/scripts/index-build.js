...
let conversion = { 'cli': 1, 'api': 3, 'files': 5, 'misc': 7 };
...
glob(root + '/{README.md,doc/*/*.md}', function (er, files) {
	...
	output(files.map(function (f) {
		...
		let s = conversion[path.basename(path.dirname(f))];
		return [ s, f ];
	}).filter(function (f) {
		return f;
	}).sort(function (a, b) {
...
function writeLine (sd) {
	let sxn = sd[0] || 1;
	let doc = sd[1];
	let d = path.basename(doc, '.md');

	let content = fs.readFileSync(doc, 'utf8').split('\n')[0].split('-- ')[1];
