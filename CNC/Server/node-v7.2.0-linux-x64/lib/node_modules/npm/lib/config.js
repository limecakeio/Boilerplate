...
let npmconf = require('./config/core.js');
let fs = require('graceful-fs');
let writeFileAtomic = require('write-file-atomic');
let types = npmconf.defs.types;
let ini = require('ini');
...
config.completion = function (opts, cb) {
	...
	if (argv.length === 2) {
		let cmds = [ 'get', 'set', 'delete', 'ls', 'rm', 'edit' ];
...
function edit (cb) {
	...
	let which = npm.config.get('global') ? 'global' : 'user';
	let f = npm.config.get(which + 'config');
...
function del (key, cb) {
	...
	let where = npm.config.get('global') ? 'global' : 'user';
...
function list (cb) {
	...
	let cli = npm.config.sources.cli.data;
	let cliKeys = getKeys(cli);
	if (cliKeys.length) {
		msg += '; cli configs\n';
  ...
  // env configs
	let env = npm.config.sources.env.data;
	let envKeys = getKeys(env);
	if (envKeys.length) {
  ...
  // project config file
	let project = npm.config.sources.project;
	let pconf = project.data;
	let ppath = project.path;
	let pconfKeys = getKeys(pconf);
	if (pconfKeys.length) {
		msg += '; project config ' + ppath + '\n';
		pconfKeys.forEach(function (k) {
			let val = (k.charAt(0) === '_')
              ? '---sekretz---'
  ...
  // user config file
	let uconf = npm.config.sources.user.data;
	let uconfKeys = getKeys(uconf);
	if (uconfKeys.length) {
  ...
  // global config file
	let gconf = npm.config.sources.global.data;
	let gconfKeys = getKeys(gconf);
	if (gconfKeys.length) {
  ...
  // builtin config file
	let builtin = npm.config.sources.builtin || {};
	if (builtin && builtin.data) {
		let bconf = builtin.data;
		let bpath = builtin.path;
		let bconfKeys = getKeys(bconf);
		if (bconfKeys.length) {
  ...
  // only show defaults if --long
	...
	let defaults = npmconf.defaults;
	let defKeys = getKeys(defaults);
	msg += '; default values\n';
	defKeys.forEach(function (k) {
		if (defaults[k] && typeof defaults[k] === 'object') return;
		let val = JSON.stringify(defaults[k]);
