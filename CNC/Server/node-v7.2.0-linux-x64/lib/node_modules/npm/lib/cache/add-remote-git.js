...
let hostedFromURL = require('hosted-git-info').fromUrl;
let inflight = require('inflight');
let log = require('npmlog');
let mkdir = require('mkdirp');
let normalizeGitUrl = require('normalize-git-url');
...
let addLocal = require('./add-local.js');
let correctMkdir = require('../utils/correct-mkdir.js');
let git = require('../utils/git.js');
let npm = require('../npm.js');
let rm = require('../utils/gently-rm.js');
let tempFilename = require('../utils/temp-filename.js');

let remotes = path.resolve(npm.config.get('cache'), '_git-remotes');
let templates = path.join(remotes, '_templates');
...
function tryClone (from, combinedURL, silent, cb) {
	...
	let normalized = normalizeGitUrl(combinedURL);
	let cloneURL = normalized.url;
	let treeish = normalized.branch;

  // ensure that similarly-named remotes don't collide
	let cachedRemote = uniqueFilename(remotes, combinedURL.replace(/[^a-zA-Z0-9]+/g, '-'), cloneURL);
	let repoID = path.relative(remotes, cachedRemote);
...
function resolveHead (from, cloneURL, treeish, cachedRemote, cb) {
	...
	let resolvedTreeish = stdout.trim();
	log.silly('resolveHead', from, 'resolved treeish:', resolvedTreeish);

	let resolvedURL = getResolved(cloneURL, resolvedTreeish);
	if (!resolvedURL) {
		return cb(new Error(
	...
	let tmpdir = path.join(tempFilename('git-cache'), resolvedTreeish);
...
function cloneResolved (from, resolvedURL, resolvedTreeish, cachedRemote, tmpdir, cb) {
	let args = [ 'clone', cachedRemote, tmpdir ];
	git.whichAndExec(
    args,
    { cwd: cachedRemote, env: gitEnv() },
    function (er, stdout, stderr) {
...
function checkoutTreeish (from, resolvedURL, resolvedTreeish, tmpdir, cb) {
	let args = [ 'checkout', resolvedTreeish ];
	git.whichAndExec(
    args,
...
function updateSubmodules (from, resolvedURL, tmpdir, cb) {
	let args = [ 'submodule', '-q', 'update', '--init', '--recursive' ];
	git.whichAndExec(
    args,
    { cwd: tmpdir, env: gitEnv() },
    function (er, stdout, stderr) {
...
function getResolved (uri, treeish) {
  ...
  // https://github.com/npm/npm/issues/7961
	let rehydrated = hostedFromURL(uri);
