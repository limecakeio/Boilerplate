...
let cacheFile = require('npm-cache-filename');
let mkdirp = require('mkdirp');
let rimraf = require('rimraf');
let chownr = require('chownr');
let writeFile = require('write-file-atomic');
let parseJSON = require('../utils/parse-json');
...
CachingRegistryClient.prototype._invalidatingRequest = function (uri, params, cb) {
	...
	this._request(uri, params, function () {
		...
		if (method !== 'HEAD' && method !== 'GET') {
			let invalidated = client._mapToCache(uri);
      // invalidate cache
      //
...
function get (uri, params, cb) {
	...
	let cacheBase = cacheFile(npm.config.get('cache'))(uri);
	let cachePath = path.join(cacheBase, '.cache.json');
...
function get_ (uri, cachePath, params, cb) {
	let staleOk = params.staleOk === undefined ? false : params.staleOk;
	let timeout = params.timeout === undefined ? -1 : params.timeout;
