...
let addRemoteTarball = require('./add-remote-tarball.js');
...
function engineFilter (data) {
	let npmv = npm.version;
	let nodev = npm.config.get('node-version');
	let strict = npm.config.get('engine-strict');

	if (!nodev || npm.config.get('force')) return data;

	Object.keys(data.versions || {}).forEach(function (v) {
		let eng = data.versions[v].engines;
		if (!eng) return;
		if (!strict) return;
...
function addNameVersion (name, v, data, cb) {
    ...
    // we got cached data, so let's see if we have a tarball.
		let pkgroot = cachedPackageRoot({ name: name, version: ver });
		let pkgtgz = path.join(pkgroot, 'package.tgz');
		let pkgjson = path.join(pkgroot, 'package', 'package.json');
        ...
        // tarballs may not work.
				let tb = url.parse(dist.tarball);
				let rp = url.parse(ruri);
...
function addNameRange (name, range, data, cb) {
    ...
    // if the tagged version satisfies, then use that.
		let tagged = data['dist-tags'][npm.config.get('tag')];
		if (tagged &&
    ...
    // find the max satisfying version.
		...
		let ms = semver.maxSatisfying(versions, range, true);
		if (!ms) {
...
function installTargetsError (requested, data) {
	let targets = Object.keys(data['dist-tags']).filter(function (f) {
	...
	let er = new Error('No compatible version found: ' + requested + '\n' + targets);
	er.code = 'ETARGET';
	return er;
}
