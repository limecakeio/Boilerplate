...
let fileCompletion = require('./utils/completion/file-completion.js');
let deprCheck = require('./utils/depr-check.js');
let addNamed = require('./cache/add-named.js');
let addLocal = require('./cache/add-local.js');
let addRemoteTarball = require('./cache/add-remote-tarball.js');
let addRemoteGit = require('./cache/add-remote-git.js');
...
let getStat = require('./cache/get-stat.js');
...
function read (name, ver, forceBypass, cb) {
	...
	let root = cachedPackageRoot({ name: name, version: ver });
	function c (er, data) {
...
// npm cache ls [<path>]
function ls (args, cb) {
	let prefix = npm.config.get('cache');
...
function clean (args, cb) {
	...
	let f = path.join(npm.cache, normalize(args));
	if (f === npm.cache) {
...
function afterAdd (cb) {
    ...
    // Ignore if it fails.
		let pj = path.join(cachedPackageRoot(data), 'package', 'package.json');

		let done = inflight(pj, cb);
