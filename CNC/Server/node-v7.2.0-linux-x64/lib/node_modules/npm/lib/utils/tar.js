...
let Packer = require('fstream-npm');
...
let myGid = process.getgid && process.getgid();
let readPackageTree = require('read-package-tree');
let union = require('lodash.union');
let moduleName = require('./module-name.js');
let packageId = require('./package-id.js');
	...
	readJson(path.join(folder, 'package.json'), function (er, pkg) {
      ...
      // in a dep of it.
			...
			readPackageTree(folder, pulseTillDone('pack:readTree:' + packageId(pkg), iferr(cb, function (tree) {
				let recalcGroup = log.newGroup('pack:recalc:' + packageId(pkg));
...
BundledPacker.prototype.applyIgnores = function (entry, partial, entryObj) {
  ...
  // package.json main file should never be ignored.
	let mainFile = this.package && this.package.main;
...
function pack_ (tarball, folder, tree, pkg, cb) {
	...
	InstancePacker.prototype.isBundled = function (name) {
		let bd = this.package && this.package.bundleDependencies;
		if (!bd) return false;
		...
		let pkg = tree.children.filter(nameMatch(name))[0];
		if (!pkg) return false;
		let requiredBy = [].concat(pkg.requiredBy);
		let seen = {};
		while (requiredBy.length) {
			let reqPkg = requiredBy.shift();
...
function gunzTarPerm (tarball, target, dMode, fMode, uid, gid, cb_) {
	...
	let fst = fs.createReadStream(tarball);
  ...
  // to be a specific user.
    ...
    // or dirs that are user-un-listable. Only leads to headaches.
		let originalMode = entry.mode = entry.mode || entry.props.mode;
	...
	let extractOpts = { type: 'Directory', path: target, strip: 1 };
    ...
    // using other means, such as system tar or git archive.
		if (this.type === 'File') {
			let base = path.basename(this.path);
			if (base === '.npmignore') {
				sawIgnores[ this.path ] = true;
			} else if (base === '.gitignore') {
				let npmignore = this.path.replace(/\.gitignore$/, '.npmignore');
	...
	cb(er);
        ...
        // naked js file
		let jsOpts = { path: path.resolve(target, 'index.js') };
	...
	cb(er);
          ...
          .on('close', function () {
	let j = path.resolve(target, 'package.json');
