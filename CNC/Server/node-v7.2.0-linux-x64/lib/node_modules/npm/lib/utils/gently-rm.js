...
let lstat = require('graceful-fs').lstat;
let readlink = require('graceful-fs').readlink;
let isInside = require('path-is-inside');
let vacuum = require('fs-vacuum');
let chain = require('slide').chain;
let asyncMap = require('slide').asyncMap;
let readCmdShim = require('read-cmd-shim');
...
function gentlyRm (target, gently, base, cb) {
  ...
  // installed globally
	...
	let targetPath = normalize(resolve(npm.prefix, target));
	...
	let options = { log: log.silly.bind(log, 'vacuum-fs') };
	...
	let parent = options.base = options.base || normalize(npm.prefix);
...
function isSafeToRm (parent, target, cb) {
  ...
  // or something the parent symlinks to
	if (target.inParent) {
		let actualTarget = target.inParent.target;
		let targetsParent = target.inParent.path;
...
function clobberFail (target, msg) {
	...
	let er = new Error('Refusing to delete ' + target + ': ' + msg);
	er.code = 'EEXIST';
	er.path = target;
	return er;
}
...
function isEverInside (target, paths, cb) {
	...
	asyncMap(paths, errorsToValues(readAllLinks), iferr(cb, function (resolvedPaths) {
		let errorFree = resolvedPaths.filter(isNotError);
		if (errorFree.length === 0) {
			let badErrors = resolvedPaths.filter(notENOENT);
...
function areAnyInsideAny (targets, paths) {
	...
	for (let ii = 0; ii < toCheck.length; ++ii) {
		let target = toCheck[ii][0];
		let path = toCheck[ii][1];
		let inside = isInside(target, path);
...
function resolveSymlink (symlink, cb) {
	validate('SF', arguments);
	let cached = resolvedPaths[symlink];
