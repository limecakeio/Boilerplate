...
[
	...
	let doExit = npm.config.get('_exit');
	if (doExit) {
    // actually exit.
...
function exit (code, noLog) {
	...
	let doExit = npm.config ? npm.config.get('_exit') : true;
...
function errorHandler (er) {
  ...
  // console.error('errorHandler', er)
	...
	let m = er.code || er.message.match(/^(?:Error: )?(E[A-Z]+)/);
	if (m && !er.code) {
		er.code = m;
	}
...
let writingLogFile = false;
function writeLogFile (cb) {
	if (writingLogFile) return cb();
	writingLogFile = true;
	wroteLogFile = true;

	let fstr = writeStreamAtomic('npm-debug.log');
	let os = require('os');
	let out = '';
