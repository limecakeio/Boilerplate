...
let getCacheStat = require('./cache/get-stat.js');
let unpack = require('./utils/tar.js').unpack;
let pulseTillDone = require('./utils/pulse-till-done.js');
...
module.exports = function fetchPackageMetadata (spec, where, tracker, done) {
	...
	let logAndFinish = andLogAndFinish(spec, tracker, done);
	if (!dep) {
...
let annotateMetadata = module.exports.annotateMetadata = function (pkg, requested, spec, where) {
...
function fetchNamedPackageData (dep, next) {
	...
	mapToRegistry(dep.name || dep.rawSpec, npm.config, iferr(next, function (url, auth) {
		...
		function pickVersionFromRegistryDocument (pkg) {
			...
			let invalidVersions = versions.filter(function (v) {
			...
			if (dep.type === 'tag') {
				let tagVersion = pkg['dist-tags'][dep.spec];
				if (pkg.versions[tagVersion]) return returnAndAddMetadata(pkg.versions[tagVersion]);
			} else {
				let latestVersion = pkg['dist-tags'][npm.config.get('tag')] || versions[0];
        ...
        // our spec
				for (let jj = 0; jj < versions.length; ++jj) {
      ...
      // And failing that, we error out
			let targets = versions.length
                  ? 'Valid install targets:\n' + versions.join(', ') + '\n'
                  : 'No valid targets found.';
			let er = new Error('No compatible version found: ' +
                         dep.raw + '\n' + targets);
			er.code = 'ETARGET';
			return next(er);
		}
...
module.exports.addShrinkwrap = function addShrinkwrap (pkg, next) {
  ...
  // FIXME: cache the shrinkwrap directly
	...
	let ver = pkg.version;
	let tarball = path.join(cachedPackageRoot({ name: pkgname, version: ver }), 'package.tgz');
		...
		untar.on('entry', function (entry) {
			...
			entry.on('end', function () {
				...
				try {
					...
					let er = new Error('Error parsing ' + pkgname + '@' + ver + "'s npm-shrinkwrap.json: " + ex.message);
					er.type = 'ESHRINKWRAP';
					return next(er);
				}
				next(null, pkg);
...
function untarStream (tarball, cb) {
	...
	let stream;
	let file = stream = fs.createReadStream(tarball);
	let tounpipe = [ file ];
	...
	file.on('data', function OD (c) {
		if (hasGzipHeader(c)) {
			...
			let er = new Error('Non-gzip/tarball ' + tarball);
			er.code = 'ENOTTARBALL';
			return cb(er);
		}
	...
	function doGunzip () {
		let gunzip = stream.pipe(zlib.createGunzip());
	...
	function doUntar () {
		let untar = stream.pipe(tar.Parse());
