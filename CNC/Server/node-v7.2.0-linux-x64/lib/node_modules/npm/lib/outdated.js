		...
		outdated_(args, '', tree, {}, 0, function (er, list) {
          ...
          aa[1].localeCompare(bb[1]);
			...
			if (npm.config.get('json')) {
				...
				let outList = list.map(makePretty);
				let outHead = [ 'Package',
					'Current',
					'Wanted',
					'Latest',
				...
				let outTable = [ outHead ].concat(outList);
...
function ansiTrim (str) {
	let r = new RegExp('\x1b(?:\\[(?:\\d+[ABCDEFGJKSTm]|\\d+;\\d+[Hfm]|' +
...
function outdated_ (args, path, tree, parentHas, depth, cb) {
	...
	let deps = tree.children.filter(function (child) {
  ...
  // FIXME: Refactor npm update to not be in terms of outdated.
	...
	let prod = npm.config.get('production') || /^prod(uction)?$/.test(npm.config.get('only'));
	if ((dev || !prod) &&
	...
	let has = Object.create(parentHas);
  ...
  // otherwise dive into the folder
	asyncMap(deps, function (dep, cb) {
		...
		let required = (tree.package.dependencies)[name] ||
  ...
  // search for the latest package
	...
	function updateLocalDeps (latestRegistryVersion) {
		readJson(path.resolve(parsed.spec, 'package.json'), function (er, localDependency) {
			...
			let wanted = localDependency.version;
			let latest = localDependency.version;
	...
	function updateDeps (er, d) {
		...
		let l = d.versions[d['dist-tags'].latest];
		if (!l) return cb();
      ...
      // see if it's in the doc.
			...
			let v = semver.maxSatisfying(vers, r, true);
			if (v) {
    ...
    // We didn't find the version in the doc.  See if cache can find it.
      ...
      // there is no newer version available
			let dFromUrl = d._from && url.parse(d._from).protocol;
			let cFromUrl = curr && curr.from && url.parse(curr.from).protocol;
