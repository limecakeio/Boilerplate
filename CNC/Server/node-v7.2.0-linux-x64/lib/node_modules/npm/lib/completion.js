...
let configTypes = configDefs.types;
let shorthands = configDefs.shorthands;
let nopt = require('nopt');
let configNames = Object.keys(configTypes)
  .filter(function (e) {
	return e.charAt(0) !== '_';
});
let shorthandNames = Object.keys(shorthands);
let allConfs = configNames.concat(shorthandNames);
...
function completion (args, cb) {
	if (isWindowsShell) {
		let e = new Error('npm completion supported only in MINGW / Git bash on Windows');
		e.code = 'ENOTSUP';
  ...
  // ie, tabbing at: npm foo b|ar
	let w = +process.env.COMP_CWORD;
	let words = args.map(unescape);
	let word = words[w];
	let line = process.env.COMP_LINE;
	let point = +process.env.COMP_POINT;
	let partialLine = line.substr(0, point);
	let partialWords = words.slice(0, w);
...
function dumpScript (cb) {
	let fs = require('graceful-fs');
	let path = require('path');
	let p = path.resolve(__dirname, 'utils/completion.sh');
...
function configCompl (opts, cb) {
	let word = opts.word;
	let split = word.match(/^(-+)((?:no-)*)(.*)$/);
	let dashes = split[1];
	let no = split[2];
	let flags = configNames.filter(isFlag);
	console.error(flags);
