...
function search (data, fields, version, title) {
	...
	if (Array.isArray(data)) {
		...
		data.forEach(function (data, i) {
			let tl = title.length;
			let newt = title.substr(0, tl - fields.join('.').length - 1) +
...
function printData (data, name, cb) {
	...
	let msg = '';
	let msgJson = [];
	let includeVersions = versions.length > 1;
	let includeFields;

	versions.forEach(function (v) {
		let fields = Object.keys(data[v]);
