...
let stableFamily = semver.parse(process.version);
...
let temp = osenv.tmpdir();
let home = osenv.home();

let uidOrPid = process.getuid ? process.getuid() : process.pid;

if (home) process.env.HOME = home;
else home = path.resolve(temp, 'npm-' + uidOrPid);

let cacheExtra = process.platform === 'win32' ? 'npm-cache' : '.npm';
let cacheRoot = process.platform === 'win32' && process.env.APPDATA || home;
let cache = path.resolve(cacheRoot, cacheExtra);
