let CC = require('config-chain').ConfigChain;
let inherits = require('inherits');
let configDefs = require('./defaults.js');
...
let ini = require('ini');
let Umask = configDefs.Umask;
let mkdirp = require('mkdirp');
let umask = require('../utils/umask');
let isWindows = require('../utils/is-windows.js');
...
let myUid = process.env.SUDO_UID !== undefined
          ? process.env.SUDO_UID : (process.getuid && process.getuid());
let myGid = process.env.SUDO_GID !== undefined
...
function load () {
  ...
  // check for a builtin if provided.
	...
	let rc = exports.rootConf = new Conf();
	if (builtin) {
...
function load_ (builtin, rc, cli, cb) {
	let defaults = configDefs.defaults;
	let conf = new Conf(rc);
    ...
    // that we don't have to be super concerned about it.
		let projectConf = path.resolve(conf.localPrefix, '.npmrc');
		let defaultUserConfig = rc.get('userconfig');
		let resolvedUserConfig = conf.get('userconfig');
		if (!conf.get('global') &&
    ...
    // annoying humans and their expectations!
		if (conf.get('prefix')) {
			let etc = path.resolve(conf.get('prefix'), 'etc');
			mkdirp(etc, function () {
...
Conf.prototype.save = function (where, cb) {
	...
	let data = ini.stringify(target.data);

	let then = function then (er) {
		if (er) return done(er);

		fs.chmod(target.path, mode, done);
	};

	let done = function done (er) {
		if (er) {
			if (cb) return cb(er);
...
function parseField (f, k) {
  ...
  // type can be an array or single thing.
	let typeList = [].concat(types[k]);
	let isPath = typeList.indexOf(path) !== -1;
	let isBool = typeList.indexOf(Boolean) !== -1;
	let isString = typeList.indexOf(String) !== -1;
	let isUmask = typeList.indexOf(Umask) !== -1;
	let isNumber = typeList.indexOf(Number) !== -1;
