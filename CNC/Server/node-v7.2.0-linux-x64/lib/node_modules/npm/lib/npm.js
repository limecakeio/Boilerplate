(function () {
  ...
  // Patch the global fs module here at the app level
	let fs = gfs.gracefulify(require('fs'));

	let EventEmitter = require('events').EventEmitter;
	let npm = module.exports = new EventEmitter();
	...
	let CachingRegClient = require('./cache/caching-client.js');
	let parseJSON = require('./utils/parse-json.js');
	let aliases = require('./config/cmd-list').aliases;
	let cmdList = require('./config/cmd-list').cmdList;
	let plumbing = require('./config/cmd-list').plumbing;
    ...
    // startup, ok to do this synchronously
		let j = parseJSON(fs.readFileSync(
	...
	let littleGuys = [ 'isntall', 'verison' ];
	let fullList = cmdList.concat(aliasNames).filter(function (c) {
		return plumbing.indexOf(c) === -1;
	});
	let abbrevs = abbrev(fullList);
  ...
  // we have our reasons
	...
	Object.keys(abbrevs).concat(plumbing).forEach(function addCommand (c) {
		Object.defineProperty(npm.commands, c, { get: function () {
          ...
          'Call npm.load(config, cb) before using this command.\n' +
			...
			let cmd = require(path.join(__dirname, a + '.js'));
	...
	function load (npm, cli, cb) {
      ...
      // look up configs
			let builtin = path.resolve(__dirname, '..', 'npmrc');
        ...
        // go ahead and spin up the registry client.
				...
				let umask = npm.config.get('umask');
				npm.modes = {
				...
				let gp = Object.getOwnPropertyDescriptor(config, 'globalPrefix');
				Object.defineProperty(npm, 'globalPrefix', gp);

				let lp = Object.getOwnPropertyDescriptor(config, 'localPrefix');
	...
	Object.defineProperty(npm, 'prefix',
		{
			...
			set: function (r) {
				let k = npm.config.get('global') ? 'globalPrefix' : 'localPrefix';
				npm[k] = r;
				return r;
			},
			enumerable: true
	...
	let tmpFolder;
	let rand = require('crypto').randomBytes(4).toString('hex');
