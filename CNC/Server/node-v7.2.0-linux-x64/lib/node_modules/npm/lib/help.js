...
let shorthands = require('./config/cmd-list').shorthands;
let commands = cmdList.concat(Object.keys(shorthands));
let output = require('./utils/output.js');

function help (args, cb) {
	let argv = npm.config.get('argv').cooked;
  ...
  // npm help foo bar baz: search topics
	...
	let section = npm.deref(args[0]) || args[0];

  // npm help <noargs>:  show basic usage
	if (!section) {
		let valid = argv[0] === 'help' ? 0 : 1;
  ...
  // npm apihelp <section>: Prefer section 3 over section 1
	let apihelp = argv.length && argv[0].indexOf('api') !== -1;
	let pref = apihelp ? [ 3, 1, 5, 7 ] : [ 1, 3, 5, 7 ];
	if (argnum) {
  ...
  // npm help <section>: Try to find the path
	let manroot = path.resolve(__dirname, '..', 'man');
  ...
  // further down.  Globs and regexps are different
	let compextglob = '.+(gz|bz2|lzma|[FYzZ]|xz)';
	let compextre = '\\.(gz|bz2|lzma|[FYzZ]|xz)$';
	let f = '+(npm-' + section + '|' + section + ').[0-9]?(' + compextglob + ')';
...
function pickMan (mans, pref_) {
	...
	mans = mans.sort(function (a, b) {
		let an = a.match(nre)[1];
		let bn = b.match(nre)[1];
...
function viewMan (man, cb) {
	let nre = /([0-9]+)$/;
	let num = man.match(nre)[1];
	let section = path.basename(man, '.' + num);

  // at this point, we know that the specified man page exists
	let manpath = path.join(__dirname, '..', 'man');
	let env = {};
	Object.keys(process.env).forEach(function (i) {
		env[i] = process.env[i];
	});
	env.MANPATH = manpath;
	let viewer = npm.config.get('viewer');
...
function htmlMan (man) {
	...
	let f = path.basename(man).replace(/([0-9]+)$/, 'html');
	switch (sect) {
		case 1:
			sect = 'cli';
			break;
...
function getSections (cb) {
	let g = path.resolve(__dirname, '../man/man[0-9]/*.[0-9]');
