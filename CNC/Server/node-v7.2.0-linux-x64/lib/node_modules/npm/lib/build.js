...
let link = require('./utils/link.js');
let linkIfExists = link.ifExists;
let cmdShim = require('cmd-shim');
let cmdShimIfExists = cmdShim.ifExists;
let asyncMap = require('slide').asyncMap;
let ini = require('ini');
let writeFile = require('write-file-atomic');
...
function build (args, global, didPre, didRB, cb) {
  ...
  // in parallel generally munges up the output from node-waf
	let builder = build_(global, didPre, didRB);
...
let writeBuiltinConf = build.writeBuiltinConf = function (pkg, folder, cb) {
  ...
  // itself globally, it puts its builtin config file there
	...
	let data = ini.stringify(npm.config.sources.builtin.data);
	writeFile(path.resolve(folder, 'npmrc'), data, cb);
};

let linkStuff = build.linkStuff = function (pkg, folder, global, didRB, cb) {
  ...
  // otherwise, then bins are in folder/../.bin
	let parent = pkg.name && pkg.name[0] === '@' ? path.dirname(path.dirname(folder)) : path.dirname(folder);
	let gnm = global && npm.globalDir;
	let gtop = parent === gnm;
...
function shouldWarn (pkg, folder, global, cb) {
	let parent = path.dirname(folder);
	let top = parent === npm.dir;
	let cwd = npm.localPrefix;

	readJson(path.resolve(cwd, 'package.json'), function (er, topPkg) {
		if (er) return cb(er);

		let linkedPkg = path.basename(cwd);
		let currentPkg = path.basename(folder);
...
function rebuildBundles (pkg, folder, parent, gtop, cb) {
	...
	let bundles = pkg.bundleDependencies || pkg.bundledDependencies || [];
...
function linkBins (pkg, folder, parent, gtop, cb) {
	...
	let binRoot = gtop ? npm.globalBin
...
function linkMans (pkg, folder, parent, gtop, cb) {
  ...
  // otherwise, if there are dupes, it'll fail with EEXIST
	let set = pkg.man.reduce(function (acc, man) {
	...
	asyncMap(pkg.man, function (man, cb) {
		...
		let manSrc = path.resolve(folder, man);
		let manDest = path.join(manRoot, 'man' + sxn, bn);
