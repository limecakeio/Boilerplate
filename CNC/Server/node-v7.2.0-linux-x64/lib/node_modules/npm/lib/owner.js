...
let readLocalPkg = require('./utils/read-local-package.js');
...
function rm (user, pkg, cb) {
	...
	mutate(pkg, user, function (u, owners) {
		let found = false;
		let m = owners.filter(function (o) {
			let match = (o.name === user);
			found = found || match;
			return !match;
		});
...
function mutate (pkg, user, mutation, cb) {
	if (user) {
		let byUser = '-/user/org.couchdb.user:' + user;
	...
	function mutate_ (er, u) {
        ...
        // out if maintainers were added or removed
				let beforeMutation = data.maintainers.length;

				let m = mutation(u, data.maintainers);
				...
				let dataPath = pkg.replace('/', '%2f') + '/-rev/' + data._rev;
