...
let symlink = require('./utils/link.js');
...
function link (args, cb) {
	if (process.platform === 'win32') {
		...
		if (!semver.gte(process.version, '0.7.9')) {
			let msg = 'npm link not supported on windows prior to node 0.7.9';
			let e = new Error(msg);
			e.code = 'ENOTSUP';
...
function linkInstall (pkgs, cb) {
	asyncMap(pkgs, function (pkg, cb) {
		let t = path.resolve(npm.globalDir, '..');
		let pp = path.resolve(npm.globalDir, pkg);
		let rp = null;
		let target = path.resolve(npm.dir, pkg);

		function n (er, data) {
			if (er) return cb(er, data);
      // we want the ONE thing that was installed into the global dir
			let installed = data.filter(function (info) {
				let id = info[0];
				let folder = info[1];
				return parentFolder(id, folder) === npm.globalDir;
			});
			let id = installed[0][0];
			pp = installed[0][1];
			let what = npa(id);
			pkg = what.name;
...
function linkPkg (folder, cb_) {
	...
	readJson(path.resolve(me, 'package.json'), function (er, d) {
		...
		let target = path.resolve(npm.globalDir, d.name);
