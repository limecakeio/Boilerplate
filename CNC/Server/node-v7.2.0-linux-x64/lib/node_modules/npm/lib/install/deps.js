...
let realizePackageSpecifier = require('realize-package-specifier');
let realizeShrinkwrapSpecifier = require('./realize-shrinkwrap-specifier');
let asap = require('asap');
let dezalgo = require('dezalgo');
let fetchPackageMetadata = require('../fetch-package-metadata.js');
let andAddParentToErrors = require('./and-add-parent-to-errors.js');
let addShrinkwrap = require('../fetch-package-metadata.js').addShrinkwrap;
let addBundled = require('../fetch-package-metadata.js').addBundled;
let readShrinkwrap = require('./read-shrinkwrap.js');
let inflateShrinkwrap = require('./inflate-shrinkwrap.js');
let inflateBundled = require('./inflate-bundled.js');
let andFinishTracker = require('./and-finish-tracker.js');
let npm = require('../npm.js');
let flatNameFromTree = require('./flatten-tree.js').flatNameFromTree;
let createChild = require('./node.js').create;
let resetMetadata = require('./node.js').reset;
let andIgnoreErrors = require('./and-ignore-errors.js');
let isInstallable = require('./validate-args.js').isInstallable;
let packageId = require('../utils/package-id.js');
let moduleName = require('../utils/module-name.js');
let isDevDep = require('./is-dev-dep.js');
let isProdDep = require('./is-prod-dep.js');
let reportOptionalFailure = require('./report-optional-failure.js');

// The export functions in this module mutate a dependency tree, adding
// items to them.

function isDep (tree, child, cb) {
	let name = moduleName(child);
	let prodVer = isProdDep(tree, name);
	let devVer = isDevDep(tree, name);
...
let registryTypes = { range: true, version: true };
...
function doesChildVersionMatch (child, requested, requestor) {
  ...
  // prereleases to match * if there are ONLY prereleases
	...
	let childReq = child.package._requested;
  ...
  // You'll see this scenario happen with at least tags and git dependencies.
	if (!registryTypes[requested.type]) {
		if (child.package._from) {
			let fromReq = npa(child.package._from);
...
function recalculateMetadata (tree, log, seen, next) {
	...
	function markDeps (toMark, done) {
		let name = toMark.name;
		let spec = toMark.spec;
		let kind = toMark.kind;
		childDependencySpecifier(tree, name, spec, function (er, req) {
			if (er || !req.name) return done();
			let child = findRequirement(tree, req.name, req);
			if (child) {
	...
	function makeMarkable (deps, kind) {
		...
		return Object.keys(deps).map(function (depname) {
			return { name: depname, spec: deps[depname], kind: kind };
		});
	}

  // Ensure dependencies and dev dependencies are marked as required
	let tomark = makeMarkable(tree.package.dependencies, 'dep');
  ...
  // Ensure any children ONLY from a shrinkwrap are also included
	let childrenOnlyInShrinkwrap = tree.children.filter(function (child) {
		return child.fromShrinkwrap &&
      !tree.package.dependencies[child.package.name] &&
      !tree.package.devDependencies[child.package.name];
	});
	let tomarkOnlyInShrinkwrap = childrenOnlyInShrinkwrap.map(function (child) {
		let name = child.package.name;
		let matched = child.package._spec.match(/^@?[^@]+@(.*)$/);
		let spec = matched ? matched[1] : child.package._spec;
		let kind = tree.package.dependencies[name] ? 'dep'
...
function packageRelativePath (tree) {
	if (!tree) return '';
	let requested = tree.package._requested || {};
	let isLocal = requested.type === 'directory' || requested.type === 'local';
...
exports.getAllMetadata = function (args, tree, where, next) {
	asyncMap(args, function (arg, done) {
		function fetchMetadataWithVersion () {
			let version = matchingDep(tree, arg);
			let spec = version == null ? arg : arg + '@' + version;
			return fetchPackageMetadata(spec, where, done);
		}
		if (tree && arg.lastIndexOf('@') <= 0) {
			let sw = getShrinkwrap(tree, arg);
			if (sw) {
...
exports.removeDeps = function (args, tree, saveToDependencies, log, next) {
	...
	args.forEach(function (pkg) {
		let pkgName = moduleName(pkg);
		let toRemove = tree.children.filter(moduleNameMatches(pkgName));
		let pkgToRemove = toRemove[0] || createChild({ package: { name: pkgName } });
		if (saveToDependencies) {
...
function andForEachChild (load, next) {
    ...
    // so shortcircuit before arg validation
		...
		let sortedCmds = cmds.sort(function installOrder (aa, bb) {
...
let failedDependency = exports.failedDependency = function (tree, name_pkg) {
	let name, pkg;
	...
	for (let ii = 0; ii < tree.requiredBy.length; ++ii) {
		let requireParent = tree.requiredBy[ii];
...
function andHandleOptionalErrors (log, tree, name, done) {
	...
	return function (er, child, childLog) {
		...
		let isFatal = failedDependency(tree, name);
		if (er && !isFatal) {
			tree.children = tree.children.filter(noModuleNameMatches(name));
...
function loadDeps (tree, log, next) {
	...
	asyncMap(Object.keys(tree.package.dependencies), function (dep, done) {
		let version = tree.package.dependencies[dep];
...
exports.loadDevDeps = function (tree, log, next) {
    ...
    // as the former
		...
		let logGroup = log.newGroup('loadDevDep:' + dep);
...
function addDependency (name, versionSpec, tree, log, done) {
	...
	let next = andAddParentToErrors(tree, done);
	childDependencySpecifier(tree, name, versionSpec, iferr(done, function (req) {
		let child = findRequirement(tree, name, req);
		if (child) {
...
let updatePhantomChildren = exports.updatePhantomChildren = function (current, child) {
	validate('OO', arguments);
	...
	return isInstallable(pkg, iferr(next, function () {
		...
		addShrinkwrap(pkg, iferr(next, function () {
			addBundled(pkg, iferr(next, function () {
				let parent = earliestInstallable(tree, tree, pkg) || tree;
				...
				delete pkg._bundled;
				let hasBundled = child.children.length;

				let replaced = replaceModuleByName(parent, 'children', child);
...
let validatePeerDeps = exports.validatePeerDeps = function (tree, onInvalid) {
	if (!tree.package.peerDependencies) return;
	Object.keys(tree.package.peerDependencies).forEach(function (pkgname) {
		let version = tree.package.peerDependencies[pkgname];
		let match = findRequirement(tree.parent || tree, pkgname, npa(pkgname + '@' + version));
...
let findRequirement = exports.findRequirement = function (tree, name, requested, requestor) {
	validate('OSO', [ tree, name, requested ]);
	if (!requestor) requestor = tree;
	let nameMatch = function (child) {
		return moduleName(child) === name && child.parent && !child.removed;
	};
	let versionMatch = function (child) {
...
let earliestInstallable = exports.earliestInstallable = function (requiredBy, tree, pkg) {
  ...
  // binaries then we need to decline to install this package here.
	let binaryMatches = pkg.bin && tree.children.some(function (child) {
  ...
  // found that version.
	let deps = tree.package.dependencies || {};
