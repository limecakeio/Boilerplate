...
function nonRegistrySource (pkg) {
	...
	let requested = pkg._requested || (pkg._from && npa(pkg._from));
...
function pkgAreEquiv (aa, bb) {
	let aaSha = (aa.dist && aa.dist.shasum) || aa._shasum;
	let bbSha = (bb.dist && bb.dist.shasum) || bb._shasum;
...
let sortActions = module.exports.sortActions = function (differences) {
	let actions = {};
	...
	let sortedlocs = Object.keys(actions).sort(sortByLocation);
  ...
  // as folks add new dependencies.
	let toplocs = sortedlocs.filter(function (location) {
		let mod = actions[location][1];
...
function diffTrees (oldTree, newTree) {
	...
	let differences = [];
	let flatOldTree = flattenTree(oldTree);
	let flatNewTree = flattenTree(newTree);
	let toRemove = {};
	let toRemoveByNameAndVer = {};
  // find differences
	Object.keys(flatOldTree).forEach(function (flatname) {
		if (flatNewTree[flatname]) return;
		let pkg = flatOldTree[flatname];
		toRemove[flatname] = pkg;
		let namever = getNameAndVersion(pkg.package);
	...
	Object.keys(flatNewTree).forEach(function (path) {
		...
		if (pkg.oldPkg) {
			...
			let vername = getNameAndVersion(pkg.package);
			if (toRemoveByNameAndVer[vername] && toRemoveByNameAndVer[vername].length && !pkg.fromBundle) {
				let flatname = toRemoveByNameAndVer[vername].shift();
