...
let isExtraneous = require('./is-extraneous.js');
...
let mutateIntoLogicalTree = module.exports = function (tree) {
...
function translateTree_ (tree, seen) {
	...
	let checkForMissingPeers = (tree.parent ? [] : [ tree ]).concat(tree.children);
	checkForMissingPeers.filter(function (child) {
		...
		Object.keys(child.missingPeers).forEach(function (pkgname) {
			let version = child.missingPeers[pkgname];
