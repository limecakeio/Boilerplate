...
let realizeShrinkwrapSpecifier = require('./realize-shrinkwrap-specifier.js');
let isRegistrySpecifier = require('./is-registry-specifier.js');
let fetchPackageMetadata = require('../fetch-package-metadata.js');
let annotateMetadata = require('../fetch-package-metadata.js').annotateMetadata;
...
function inflateShrinkwrap (topPath, tree, swdeps, finishInflating) {
	...
	tree.children = [];
	let dev = npm.config.get('dev') || (!/^prod(uction)?$/.test(npm.config.get('only')) && !npm.config.get('production')) || /^dev(elopment)?$/.test(npm.config.get('only'));
	let prod = !/^dev(elopment)?$/.test(npm.config.get('only'));
	...
	function andInflate (name, next) {
		return function (requested) {
			let sw = swdeps[name];
			let dependencies = sw.dependencies || {};
			if ((!prod && !sw.dev) || (!dev && sw.dev)) return next();
			let child = onDisk[name];
			if (childIsEquivalent(sw, requested, child)) {
				...
				let from = sw.from || requested.raw;
