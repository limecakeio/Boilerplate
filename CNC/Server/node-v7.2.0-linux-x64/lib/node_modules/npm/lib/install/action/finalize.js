...
module.exports = function (top, buildpath, pkg, log, next) {
	...
	let delpath = path.join(path.dirname(pkg.path), '.' + path.basename(pkg.path) + '.DELETE');
    ...
    // succeed or fail though we want the original error that caused this
		if (renameEr) return rename(delpath, pkg.path, function () {
			next(renameEr);
		});
	...
	function moveModules (mkdirEr, files) {
		...
		asyncMap(files, function (file, done) {
			let from = path.join(delpath, 'node_modules', file);
...
module.exports.rollback = function (buildpath, pkg, next) {
	let top = path.resolve(buildpath, '..');
