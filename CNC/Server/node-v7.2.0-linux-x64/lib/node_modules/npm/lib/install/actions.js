...
let failedDependency = require('./deps.js').failedDependency;
...
function andHandleOptionalDepErrors (pkg, next) {
	return function (er) {
		...
		markAsFailed(pkg);
		let anyFatal = pkg.userRequired || pkg.isTop;
		for (let ii = 0; ii < pkg.requiredBy.length; ++ii) {
			let parent = pkg.requiredBy[ii];
			let isFatal = failedDependency(parent, pkg);
...
function prepareAction (staging, log) {
	...
	return function (action) {
		...
		let top = path.resolve(staging, '../..');
		let buildpath = buildPath(staging, pkg);
